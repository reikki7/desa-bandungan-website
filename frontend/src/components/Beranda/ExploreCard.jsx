...
const ExploreCard = ({ title, description, image, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-0 z-10 flex flex-col gap-1 p-3 text-white text-start"
        style={{ pointerEvents: "none" }}
      >
        <h2 className="font-bold" style={{ fontSize: fontSize }}>
          {title}
        </h2>
        <p
          className={`hidden text-sm opacity-75 ${hovered ? "lg:block" : "lg:hidden"}`}
