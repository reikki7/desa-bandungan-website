        ...
        switch (filter) {
          case "Kegiatan Lalu":
            filtered = data.filter(
              (kegiatan) => kegiatan.type_kegiatan === "Kegiatan Lalu"
            );
            break;
          case "Kegiatan Mendatang":
            filtered = data.filter(
              (kegiatan) => kegiatan.type_kegiatan === "Kegiatan Mendatang"
            );
            break;
          default:
            filtered = data;
      ...
      <div className="flex flex-col items-center justify-center mx-4 mt-5">
        ...
        {isMobile ? (
          ...
          <div className="relative flex items-center justify-center mt-5 mr-4 space-x-4 content">
            <div
              className={`duration-[360ms] transition-all text-[#004b23] font-bold text-nowrap text-center absolute flex justify-center z-10 items-center text-sm bg-white h-12 rounded-full ${filter === "none" ? "w-20 mr-[317px]" : filter === "Kegiatan Lalu" ? "w-32 mr-[100px]" : "w-44 -mr-[254px]"}`}
            >
              <div
          ...
          (searchQuery.length > 0 ? searchResults : displayedData).map(
            (kegiatan) => {
              const filteredImages = imageLinks.filter(
                (image) => image.event_id === kegiatan.id
              );
              const imageUrl =
                filteredImages.length > 0 ? filteredImages[0].name : null;
              return (
                <InfoCard
                  key={kegiatan.id}
                  id={kegiatan.id}
